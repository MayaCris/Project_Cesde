...
while int(len(list_camp_cars)) < camp_cars:
    list_camp_cars.append({"place": input("Ingrese la identificación del lugar a asignar para carro : "), "avalible": True})

while int(len(list_camp_moto)) < camp_moto:
    list_camp_moto.append({"place": input("Ingrese la identificación del lugar a asignar para moto : "), "avalible": True})
...
def enter_vehicle():
    ...
    if type_vehicle == 1:
        ...
        register_car.append({"placa":plate, "espacio_asignado": assing_place["place"], "hora_entrada_carro": hour_start_car})
        for campo in list_camp_cars:
            if campo["place"] == assing_place["place"]:
                campo["avalible"] = False
                print(f'El vehiculo con placa {plate} se encuentra en el espacio {assing_place}  e ingresó a la hora {hour_start_car}')
                return True
        ...
        for campo in list_camp_moto:
            if campo["place"] == assing_place["place"]:
                campo["avalible"] = False
                print(f'El vehiculo con placa {plate} se encuentra en el espacio {assing_place}  e ingresó a la hora {hour_start_moto}')
                return True
